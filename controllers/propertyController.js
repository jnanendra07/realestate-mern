...
propertyController.get('/find/types', async (req, res) => {
    try {
        ...
        const individualType = await Property.countDocuments({ type: 'individual' })
        const industrialType = await Property.countDocuments({ type: 'industrial' })
        const rentType = await Property.countDocuments({ type: 'rent' })
        const landType = await Property.countDocuments({ type: 'land' })

        return res.status(200).json({ apartment: apartmentType, mansion: mansionType, individual: individualType,industrial: industrialType,rent: rentType,land: landType})
    } catch (error) {
        return res.status(500).json(error)
    }
})
